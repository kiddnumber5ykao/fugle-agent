...
**重要:看到「整理」「整理一下」「看一下」「現在狀況」「我的股票怎樣」「該動哪些」「我該不該停利」**
**「最近哪些可以買」「我的觀察清單」這些觸發詞,一律用下方 organize_* 工具,**
**絕對不要用 sync_portfolio_from_trades 或 valuate_portfolio(它們不會算 RSI / 訊號摘要 / AI 建議)。**

- organize_all: 使用者**單獨**說「整理一下」「整理」「幫我看一下」(沒指明追蹤還是部位)
  → **直接呼叫這個**,一次整理兩張分頁。**這是 90% 情境下的正解。**
- organize_positions: 使用者說「整理一下我的部位」「我的股票怎樣」「我該動哪些」「該不該停利/停損」
  → 對股票部位每一檔抓即時價 + 算市值/損益/損益% + 5 個技術訊號 + 寫訊號摘要 + AI 建議。
  AI 建議是 🔴 全部停利 / 🟠 停利 1/2 / 🟡 停利 1/4 / 🟢 可加碼 / ❌ 考慮停損 / ⚠️ 警戒 / 🔵 續抱。
- organize_watchlist: 使用者說「整理一下追蹤清單」「我的觀察清單怎樣」「最近哪些可以買」
  → 對追蹤清單每一檔抓 K 線、算 5 個訊號、寫摘要 + AI 建議(✅ 可進場 / 🟡 觀察 / ❌ 不建議)。
