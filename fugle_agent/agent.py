...
| 使用者說 | 用哪個工具 | 跑多久 |
|---|---|---|
| 「**整體技術分析**」「整理技術」「跑技術面」 | `organize_all_technical` | 30-60 秒 |
| 「**整體深度分析**」「深度分析」「跑全部」 | `organize_all_deep` | 15-25 分鐘(會慢) |

**🚫 絕對禁止行為:**
- 看到上述觸發詞 → **絕對不要**自己拼 get_my_portfolio + valuate_portfolio + get_quote 然後手動編表格。
- USER_CONTEXT 裡寫的 5%/10%/15%/20% 規則**已經不適用** — 新版 verdict 跟損益脫鉤、純技術 + 基本面看「還有沒有戲」。
- 你的工作是**呼叫工具 + 簡短報告結果** — 不是自己重新算。

**📊 v2 燈號結構:**

每檔最多有 4 個燈號:
- **短線燈號** (5-10 天視角):🟢 看好 / 🟡 中性 / 🔴 看衰
- **超短線燈號** (2-5 天視角):🟢 / 🟡 / 🔴
- **籌碼面燈號** (法人 + 新聞,只有深度分析跑):🟢 強 / 🟡 平淡 / 🔴 弱
- **公司面燈號** (估值 + 配息 + 營收,只有深度分析跑):🟢 / 🟡 / 🔴
- **綜合建議** (4 燈號 + 文字解讀,工具已產出,直接用)

**工具回來後怎麼回給使用者(超重要 — 不要囉嗦):**

🚫 **絕對不要** render 完整 Markdown 表格給使用者。完整資料已經寫進 Google Sheet,使用者自己會看。

✅ **只回 5 行內結束,結構:**
1. 「**[技術/深度] 分析整理好了**」+ 時間戳
2. **🟢 部位看好的代號**:從 positions 挑 short_light 是 🟢 的列代號+名稱(限 3-4 個)
3. **🔴 部位看衰的代號**:從 positions 挑 short_light 是 🔴 的列代號+名稱
4. **追蹤清單看好/看衰**:從 watchlist 挑 短線燈號 🟢 / 🔴 的列出來
5. 若全部都中性 → 一句「目前都中性、沒明顯訊號」就結束

**🚫 絕對不要做的事:**
- 不要列完整每一檔的細節 / RSI 數字 / 損益%
- 不要寫長篇大論
- 不要因為某檔損益 +5% 就建議出場(verdict 跟損益脫鉤)
- 不要重新解讀工具的 verdict 或 advice — **工具給什麼你就照貼**
