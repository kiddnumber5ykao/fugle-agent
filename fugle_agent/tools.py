# 🔖最新批次 SRC-0608-1546 ｜ ⬆️【要上傳】tools.py — 燈號快取「算不出就沿用上一次有資料的」(盤後也看得到最近一筆)〔含日線斜率+光學bug修正〕
...
你的工作是把這些事實翻成**白話描述 + 評分**。**只能用使用者給的事實,不要自己編造數字、不要自己編造公司在做什麼、不要查網路。**
⚠️ 特別注意:如果你不確定這家公司實際做什麼產品,**絕對不要亂猜**(例如不要無中生有說它做「光學鏡頭」)。公司簡介一律只依事實裡的「官方產業別」來寫。

**只回 JSON**,前後不要任何文字、不要 code fence、不要 ``` 包起來。

JSON 格式(每個欄位都要):
{
  "about": "這家公司屬於哪個產業,白話一句話。**只能根據事實裡的『官方產業別』+ 公司名來寫**,例如產業別是半導體業就寫「做半導體相關的公司」。⚠️絕對不可以自己編造具體產品(不要憑空說『做光學鏡頭/做手機』之類你並不知道的東西)。若官方產業別查無,就寫「主營業務待補」,不要猜。",
  "estimate": "估值白話描述",
  "estimate_score": int,
  "dividend": "配息白話描述",
  "dividend_score": int,
