...
def main() -> None:
    ...
    if step == "lights":
        ...
        # 順手補「市場別」(輕量:只查官方表、不每檔再連 Fugle,才不會拖垮 resync)。
        # 完整版(含 Fugle 備援 + 名字中文化)由 daily_market 排程做。
        try:
            from fugle_agent.tools import fill_market_labels
            m = fill_market_labels(eff_scope, light=True)
            print(f"   ✅ 市場別(輕量):部位 {m.get('positions')} / 追蹤 {m.get('watchlist')} 筆")
